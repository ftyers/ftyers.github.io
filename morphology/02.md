...
```
$ ls -l grn.lexc.hfst 
-rw-r--r-- 1 fran fran 491 des  3 19:57 grn.lexc.hfst
```
...
In finite-state transducers the the morphotactic ordering constraints are implemented by means of continuation classes. These are sets of suffixes which can appear in the same position. For example let's suppose we want to add the genetive case suffix *-gui* in Guaraní. 
...
- `CASE`: Here we define genetive case suffix and say that this is (for now) the end of the word.
...
```
cat grn.lexc | python3 lexc2dot.py | dot -Tpng -o grn.lexc.png
```
...
hfst-fst2strings grn.lexc.hfst     
irũ<n>:irũ
irũ<n>+gui<post>:irũ>gui
ava<n>:ava
ava<n>+gui<post>:ava>gui
apyka<n>:apyka
apyka<n>+gui<post>:apyka>gui
óga<n>:óga
óga<n>+gui<post>:óga>gui
```
...
**WARNING!** don't forget to define %{m%} as a multicharacter symbol. That means add %{m%}to the `Multichar_Symbols` section at the top of the file. 
...
irũ<n>:irũ
irũ<n>+gui<gen>:irũ>gui
irũ<n>+pe<loc>:irũ>{m}e
apyka<n>:apyka
apyka<n>+gui<gen>:apyka>gui
apyka<n>+pe<loc>:apyka>{m}e
ava<n>:ava
ava<n>+gui<gen>:ava>gui
ava<n>+pe<loc>:ava>{m}e
óga<n>:óga
óga<n>+gui<gen>:óga>gui
óga<n>+pe<loc>:óga>{m}e
...
apyka<n>+gui<gen>:apykagui
apyka<n>+pe<loc>:apykame
apyka<n>+pe<loc>:apykape
apyka<n>:apyka
ava<n>+gui<gen>:avagui
ava<n>+pe<loc>:avame
ava<n>+pe<loc>:avape
ava<n>:ava
irũ<n>+gui<gen>:irũgui
irũ<n>+pe<loc>:irũme
irũ<n>+pe<loc>:irũpe
irũ<n>:irũ
óga<n>+gui<gen>:ógagui
óga<n>+pe<loc>:ógame
óga<n>+pe<loc>:ógape
óga<n>:óga
```

As you can see, now we have all of the possible forms, this is the **key** to `twol`, we first expand all the possibilities and then constrain them. So, if we want to write a constraint for nasalization, what might it look like? First we have to define what nasalization means. 

The archiphoneme %{m%} should be `-m-` after a nasal sound, and it should be `-p-` after all other non-nasal sounds.

The first thing we should do is define some sets for what nasals mean: 
...
```
hfst-fst2strings -n 5 arn.lexc.hfst
wangku<n>:wangku
wangku<n><cmp>+che<n>:wangku>#che
wangku<n><cmp>+che<n><cmp>+mamüll<n>:wangku>#che>#mamüll
wangku<n><cmp>+che<n><cmp>+mamüll<n><cmp>+wangku<n>:wangku>#che>#mamüll>#wangku
wangku<n><cmp>+che<n><cmp>+mamüll<n><cmp>+wangku<n><cmp>+che<n>:wangku>#che>#mamüll>#wangku>#che


hfst-fst2strings -r 5 arn.lexc.hfst
...
```
echo tembiasa | hfst-lookup -qp grn.mor.hfst 
tembiasa	tembiasa<n>	0,000000
```

**Tip:** If you don't have the file `grn.mor.hfst`, use `hfst-invert` on `grn.gen.hfst`. You can `hfst-fst2strings -r 10` to check the result. 
