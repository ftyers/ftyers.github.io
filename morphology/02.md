...
**Tip:** If you don't have the file `grn.mor.hfst`, use `hfst-invert` on `grn.gen.hfst`. You can `hfst-fst2strings -r 10` to check the result, e.g. the analyses should be on the right of the `:`.
...
```
$ echo tembiasakue | hfst-lookup -qp chv.mor.hfst 
tembiasakue	tembiasakue<n>	0,000000
tembiasakue	tembiasa<n><der_kue>	1,000000

$ echo tembiasakue | hfst-lookup -qp -b 0 chv.mor.hfst 
tembiasakue	tembiasakue<n>	0,000000

$ echo "ógakue" | hfst-lookup -qp -b 0 chv.mor.hfst 
