...
Guaraní local nasalisation rules do not work for Spanish loans. Thus, we have *urũme* but *asociaciónpe*, locative does not change. How to deal with that? Introduce some archiphoneme that will "cancel" the nasalization rule.
...
```
Now we can add an exception to nasalization rule we defined earlier in `twol` file. This way the transducer will overlook the cases where the nasalization is not needed.
